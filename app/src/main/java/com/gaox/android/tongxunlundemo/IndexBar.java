...
public class IndexBar extends View {
    ...
    private List<? extends CarBaseBean> mSourceDatas;//Adapter的数据源
        ...
        setmOnIndexPressedListener(new onIndexPressedListener() {
            ...
            public void onIndexPressed(int index, String text) {
                ...
                if (mLayoutManager != null) {
                    int position = mPositionList.get(index);
                    if (position != -1) {
    ...
    public IndexBar setmSourceDatas(List<? extends CarBaseBean> mSourceDatas) {
    ...
    private void initSourceDatas() {
        ...
        if (isNeedRealIndex) {
            mIndexDatas.add("#");
            mPositionList.add(0);
            getSortedIndexDatas(mSourceDatas, mIndexDatas);

            computeGapHeight();
        }
        Log.i(TAG, mIndexDatas.toString());
        Log.i(TAG, mPositionList.toString());
        //sortData();
    }


    public void getSortedIndexDatas(List<? extends CarBaseBean> sourceDatas, List<String> indexDatas) {
        ...
        for (int i = 0; i < sourceDatas.size(); i++) {
            if (sourceDatas.get(i).getItemType() == 1) {
                //如果是tittle
